...
def proxy_by_uri(uri, jump):
    ...
    if loc:
        host_name, port = proto.netloc_split(loc, default_port=22 if 'ssh' in rawprotos else 9090)
    else:
        host_name = port = None
...
def main(args = None):
    ...
    parser.add_argument('-l', dest='listen', default=[], action='append', type=proxies_by_uri, help='tcp server uri (default: http+socks4+socks5://:9090/)')
    ...
    if not args.listen and not args.ulisten:
        args.listen.append(proxies_by_uri('http+socks4+socks5://:9090/'))
    args.httpget = {}
    if args.pac:
